...
export(int) var lightning_count: int = 1
...
func apply(player_index: int) -> void:
    ...
    effects[key_hash].append([lightning_count, chance, self])

func unapply(player_index: int) -> void:
    if key_hash == Keys.empty_hash: return

    var effects: Dictionary = RunData.get_player_effects(player_index)
    effects[key_hash].erase([lightning_count, chance, self])
...
func serialize() -> Dictionary:
    ...
    serialized.lightning_count = lightning_count
...
func deserialize_and_merge(serialized: Dictionary) -> void:
    ...
    stats = struct_stats
    lightning_count = serialized.lightning_count
