...
var _yztato_chimera_texture_sets: Array = []
var _yztato_active_boomerangs: Array = []
var _yztato_max_damage_mul: float = 0.0
var _yztato_is_boomerang: bool = false
var _yztato_is_returning: bool = false
var _yztato_knockback_only_back: bool = false
...
func on_weapon_hit_something(thing_hit: Node, damage_dealt: int, hitbox: Hitbox) -> void:
    ...
    _yztato_challenge_on_hit(hitbox)
...
func _yztato_init_chimera_stats() -> void:
    for effect in effects:
        ...
        for proj_stats in effect.chimera_projectile_stats:
            ...
            if sprite_node:
                _yztato_chimera_texture_paths.append(sprite_node.texture.resource_path)
            projectile_instance.queue_free()
            _yztato_chimera_projs.append(WeaponService.init_ranged_stats(proj_stats, player_index))
        for proj_texture_set in effect.chimera_texture_sets:
            _yztato_chimera_texture_sets.append(proj_texture_set)

        break

func _yztato_setup_boomerang() -> void:
    for effect in effects:
        if effect.get_id() != "yztato_boomerang_weapon":
            continue

        _yztato_is_boomerang = true
...
func _yztato_boomerang_on_projectile_shot(projectile: Node2D) -> void:
    if not _yztato_is_boomerang:
        return

    _yztato_active_boomerangs.append(projectile)
    _hitbox.damage *= 1 + _yztato_max_damage_mul
    if _yztato_knockback_only_back:
        _hitbox.set_knockback(Vector2.ZERO, 0.0, 0.0)
    if not projectile.is_connected("returned_to_player", self, "_yztato_on_projectile_returned"):
        projectile.connect("returned_to_player", self, "_yztato_on_projectile_returned")
...
func _yztato_challenge_on_hit(hitbox: Hitbox) -> void:
    if hitbox == null:
        return
...
func _yztato_on_projectile_returned(projectile: Node2D) -> void:
    ...
    if _yztato_active_boomerangs.empty():
        _yztato_is_returning = false
        if _yztato_wait_until_return:
            _current_cooldown = get_next_cooldown()
