...
var _yztato_blade_storm_initialized: bool = false
...
func take_damage(value: int, args: TakeDamageArgs) -> Array:
    var result: Array = .take_damage(value, args)
...
func heal(value: int, is_from_torture: bool = false) -> int:
    ...
    if _is_dying and current_stats.health > 0 and not dead:
        _yztato_cancel_delayed_death()

    return healed

func _on_LoseHealthTimer_timeout() -> void:
    if _yztato_lose_hp_per_second_min_hp():
        return
...
func _yztato_blood_rage_ready() -> void:
    ...
    if not blood_rage_effects.empty():
        for i in blood_rage_effects.size():
            ...
            blood_rage_timer.wait_time = effect[0]
...
func _yztato_heal_on_damage_taken(result: Array) -> void:
    ...
    for effect in heal_on_damage_taken:
        ...
        if Utils.get_chance_success(chance):
            ...
            if heal_amount > 0:
                on_healing_effect(heal_amount, tracking_key_hash)
...
func _yztato_projectiles_on_hurt(result: Array) -> void:
    ...
    var entity_spawner: EntitySpawner = Utils.get_scene_node()._entity_spawner
...
func _yztato_lose_hp_per_second_min_hp() -> bool:
    ...
    if lose_hp_per_second_min_hp <= 0:
        return false
    ...
    if lose_hp_per_second > 0:
        take_damage(lose_hp_per_second, _take_damage_args)
    elif lose_hp_per_second < 0:
        on_healing_effect(-lose_hp_per_second)
...
func _yztato_delayed_death_process(delta: float) -> void:
    ...
    if _death_delay < _death_delay_duration:
        return
...
func _yztato_finish_delayed_death() -> void:
    ...
    set_process(false)
    modulate = Color(1, 1, 1, 1)
    if is_queued_for_deletion() or cleaning_up or dead:
        return
